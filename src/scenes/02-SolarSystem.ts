...
export default class SolarSystemScene extends Scene {
    ...
    private drawSystem(parent: mat4, system: SolarSystemDescription){
        ...
        let universe = mat4.clone(parent);
        mat4.rotateY(universe, universe, this.time*system.rotationSpeedAroundSelf);
        mat4.scale(universe, universe, [system.scale, system.scale, system.scale]);
        ...
        if (system.children) {
            ...
            for (var j = 0; j < system.children.length; j++) {
                let universe = mat4.clone(parent);
                mat4.rotateY(universe, universe, this.time * system.children[j].rotationSpeedAroundParent);
                mat4.translate(universe, universe, [system.children[j].distanceFromParent, 0, 0]);
                mat4.scale(universe, universe, [system.children[j].scale, system.children[j].scale, system.children[j].scale]);
                mat4.rotateY(universe, universe, (this.time * system.children[j].rotationSpeedAroundSelf));
                this.drawSphere(universe, system.children[j].tint);
                if (system.children[j].children) {
                    for (var i = 0; i < system.children[j].children.length; i++) {
                        let universe = mat4.clone(parent);
                        mat4.rotateY(universe, universe, this.time * system.children[j].rotationSpeedAroundParent);
                        mat4.translate(universe, universe, [system.children[j].distanceFromParent, 0, 0]);
                        mat4.rotateY(universe, universe, this.time * system.children[j].children[i].rotationSpeedAroundParent);
                        mat4.translate(universe, universe, [system.children[j].children[i].distanceFromParent, 0, 0]);
                        mat4.scale(universe, universe, [system.children[j].children[i].scale, system.children[j].children[i].scale, system.children[j].children[i].scale]);
                        mat4.rotateY(universe, universe, (this.time * system.children[j].children[i].rotationSpeedAroundSelf));
                        this.drawSphere(universe, system.children[j].children[i].tint);
                        if (system.children[j].children[i].children) {
                            for (var k = 0; k < system.children[j].children[i].children.length; k++) {
                                ...
                                mat4.rotateY(universe, universe, this.time * system.children[j].children[i].rotationSpeedAroundParent);
                                mat4.translate(universe, universe, [system.children[j].children[i].distanceFromParent, 0, 0]);
                                mat4.rotateY(universe, universe, this.time * system.children[j].children[i].children[k].rotationSpeedAroundParent);
                                mat4.translate(universe, universe, [system.children[j].children[i].children[k].distanceFromParent, 0, 0]);
                                mat4.scale(universe, universe, [system.children[j].children[i].children[k].scale, system.children[j].children[i].children[k].scale, system.children[j].children[i].children[k].scale]);
                                mat4.rotateY(universe, universe, (this.time * system.children[j].children[i].children[k].rotationSpeedAroundSelf));
                                this.drawSphere(universe, system.children[j].children[i].children[k].tint);
                            }
                        }
                    }
                }
